...
class ModifiedLSTMCell(LayerRNNCell):
    def __init__(self,
                 num_units,
                 hidden_dim,
                 train,
                 ...
                 name=None,
                 dtype=None):
        super(ModifiedLSTMCell, self).__init__(_reuse=reuse, name=name, dtype=dtype)
        if not state_is_tuple:
            logging.warn("%s: Using a concatenated state is slower and will soon be "
                         "deprecated.  Use state_is_tuple=True.", self)
        ...
        if train == 1:
            self.W_decomp = self.init_weights(self._hidden_dim, self._hidden_dim, name='Decomposition_Hidden_weight',reg=None)
            self.b_decomp = self.init_bias(self._hidden_dim, name='Decomposition_Hidden_bias_enc')
        else:
            self.W_decomp = self.no_init_weights(self._hidden_dim, self._hidden_dim, name='Decomposition_Hidden_weight')
            self.b_decomp = self.no_init_bias(self._hidden_dim, name='Decomposition_Hidden_bias_enc')
    ...
    def call(self, inputs, state):
        ...
        time, length, _ = tf.split(inputs, [1, 1, 160], 1)
        T = self.map_elapse_time(time)

        c_st = tf.nn.tanh(tf.matmul(c,self.W_decomp)+self.b_decomp)
        c_st_ds=tf.multiply(T,c_st)
        c=tf.add(tf.subtract(c,c_st),c_st_ds)
    ...
    def map_elapse_time(self, t):
        ...
        Ones = tf.ones([1, self._hidden_dim], dtype=tf.float32)
...
# LSTMCell相比于BasicLSTMCell多了use_peepholes
